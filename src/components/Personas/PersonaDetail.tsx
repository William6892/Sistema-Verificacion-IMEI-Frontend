...
interface PersonaDetailProps { onBack?: () => void; }
...
const InfoRow = ({ label, value }: { label: string; value: React.ReactNode }) => (
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '12px 0', borderBottom: `1px solid ${tk.border}` }}>
    <span style={{ fontSize: '13px', color: tk.textSub, fontWeight: 500, flexShrink: 0, marginRight: '16px' }}>{label}</span>
    <span style={{ fontSize: '14px', color: tk.text, fontWeight: 600, textAlign: 'right' }}>{value || '—'}</span>
  </div>
);
...
const PersonaDetail: React.FC<PersonaDetailProps> = ({ onBack }) => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const p = await personasService.getPersona(parseInt(id!));
      setPersona(p);
      if (p.empresaId) { try { setEmpresa(await empresasService.getEmpresa(p.empresaId)); } catch {} }
      try { setDispositivos(await dispositivosService.getDispositivosPorPersona(parseInt(id!))); } catch {}
  ...
  const formatDate = (ds: string) => {
    if (!ds) return '—';
    return new Date(ds).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' });
  };
  ...
  if (error || !persona) return (
    <div style={{ background: tk.white, borderRadius: '16px', padding: '60px 40px', textAlign: 'center', maxWidth: '560px', margin: '40px auto', boxShadow: tk.shadow, border: `1px solid ${tk.borderMd}` }}>
      ...
      <HovBtn base={{ padding: '11px 22px', background: `linear-gradient(135deg, ${tk.blue}, ${tk.blueDark})`, color: tk.white, border: 'none', borderRadius: '9px', cursor: 'pointer', fontWeight: 700, fontSize: '14px', display: 'inline-flex', alignItems: 'center', gap: '8px', transition: 'all 0.2s' }} hover={{ transform: 'translateY(-1px)' }} onClick={goBack}>← Volver</HovBtn>
    </div>
  );
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto', fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '22px', flexWrap: 'wrap', gap: '12px' }}>
        ...
        {isAdmin && (
          <div style={{ display: 'flex', gap: '8px' }}>
            <HovBtn base={{ ...btnBase, background: tk.greenBg, color: tk.green, border: `1px solid ${tk.greenBorder}` }} hover={{ background: '#d9f7be', transform: 'translateY(-1px)' }} onClick={() => navigate(`/personas/editar/${id}`)}>✏️ Editar</HovBtn>
        ...
        <div style={{ padding: '24px 28px', animation: 'fadeUp 0.25s ease' }}>
          {activeTab === 'info' ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '18px' }}>
              {[
                { title: '📄 Información Básica', icon: tk.blue, bg: tk.blueLight, rows: [
                ...
                { title: '🏢 Empresa', icon: tk.purple, bg: tk.purpleBg, rows: [
                ...
                { title: '⚙️ Sistema', icon: tk.orange, bg: tk.orangeBg, rows: [
                ...
                <div key={section.title} style={{ background: tk.bg, borderRadius: '12px', border: `1px solid ${tk.borderMd}`, overflow: 'hidden' }}>
                  <div style={{ padding: '13px 16px', background: section.bg, borderBottom: `1px solid ${tk.borderMd}`, display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px', flexWrap: 'wrap', gap: '10px' }}>
                ...
                {isAdmin && (
                  <HovBtn base={{ ...btnBase, background: `linear-gradient(135deg, ${tk.blue}, ${tk.blueDark})`, color: tk.white, boxShadow: '0 4px 10px rgba(24,144,255,0.3)' }} hover={{ transform: 'translateY(-1px)', boxShadow: '0 6px 14px rgba(24,144,255,0.4)' }} onClick={() => navigate(`/dispositivos/asignar?personaId=${id}`)}>
                    + Asignar dispositivo
                  </HovBtn>
                )}
              </div>
              ...
              {dispositivos.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '50px 20px', background: tk.bg, borderRadius: '12px', border: `2px dashed ${tk.borderMd}` }}>
                  ...
                  {isAdmin && <HovBtn base={{ ...btnBase, background: tk.blueLight, color: tk.blue, border: `1px solid ${tk.blueBorder}` }} hover={{ background: '#bae7ff' }} onClick={() => navigate(`/dispositivos/asignar?personaId=${id}`)}>+ Asignar primer dispositivo</HovBtn>}
                </div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '14px' }}>
                  {dispositivos.map(dev => (
                    <div key={dev.id} onMouseEnter={() => setHovCard(dev.id)} onMouseLeave={() => setHovCard(null)}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: '14px' }}>
        {[
          { icon: '📱', value: dispositivos.length, label: 'Dispositivos asignados', color: tk.blue, bg: tk.blueLight },
          { icon: '✅', value: activeDevices,        label: 'Dispositivos activos',   color: tk.green, bg: tk.greenBg },
          { icon: '🏢', value: empresa ? 1 : 0,     label: 'Empresa asignada',        color: tk.purple, bg: tk.purpleBg },
        ].map(s => {
          const [h, setH] = useState(false);
          return (
            <div key={s.label} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
              style={{ background: h ? s.bg : tk.white, border: `2px solid ${h ? s.color : tk.borderMd}`, borderRadius: '14px', padding: '20px', display: 'flex', alignItems: 'center', gap: '16px', boxShadow: h ? tk.shadow : 'none', transition: 'all 0.2s', transform: h ? 'translateY(-3px)' : 'none' }}>
              <div style={{ width: '52px', height: '52px', background: `linear-gradient(135deg, ${s.color}, ${s.color}cc)`, borderRadius: '13px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', flexShrink: 0 }}>{s.icon}</div>
              <div>
                <div style={{ fontSize: '28px', fontWeight: 800, color: s.color, lineHeight: 1 }}>{s.value}</div>
                <div style={{ fontSize: '13px', color: tk.textSub, fontWeight: 500, marginTop: '4px' }}>{s.label}</div>
              </div>
            </div>
          );
        })}
